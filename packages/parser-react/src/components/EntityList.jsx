...
import { variable as v, WC, word as w } from "@bassline/parser/types";
...
export function EntityList({ events, onSelect, filter, searchTerm = "" }) {
    ...
    const entityPattern = useMemo(() => {
        return pattern(pq(v("entity"), WC, WC, WC));
    }, [WC]);
    ...
    const entities = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            entityArray = entityArray.filter((entity) => {
                const serialized = entity.toString().toLowerCase();
            ...
            >
                {entities.map((entity, index) => (
                    <div
                        ...
                        style={{
                            padding: "12px 16px",
                            borderBottom: index < entities.length - 1
                                ? "1px solid #f1f5f9"
                                : "none",
