...
describe("Parser - Basic Values", () => {
    ...
    it("parses strings", () => {
        ...
        const str = result.buffer[0];
        expect(str).toBeInstanceOf(StringCell);
        expect(str.buffer.join("")).toBe("hello world");
    });

    it("parses strings with escape sequences", () => {
        const result = parse('"hello\\nworld"');
        const str = result.buffer[0];
        expect(str.buffer.join("")).toBe("hello\nworld");
    });

    it("parses empty strings", () => {
        const result = parse('""');
        const str = result.buffer[0];
        expect(str.buffer.join("")).toBe("");
    });
});
...
describe("Parser - Series", () => {
    it("parses empty blocks", () => {
        const result = parse("[]");
        const block = result.buffer[0];
        expect(block).toBeInstanceOf(BlockCell);
        expect(block.buffer.length).toBe(0);
    });
    ...
    it("parses blocks with values", () => {
        ...
        const block = result.buffer[0];
        expect(block).toBeInstanceOf(BlockCell);
        expect(block.buffer.length).toBe(3);
        expect(block.buffer[0].value).toBe(1);
        expect(block.buffer[1].value).toBe(2);
        expect(block.buffer[2].value).toBe(3);
    });

    it("parses nested blocks", () => {
        const result = parse("[[1 2] [3 4]]");
        const block = result.buffer[0];
        expect(block.buffer.length).toBe(2);
        expect(block.buffer[0]).toBeInstanceOf(BlockCell);
        expect(block.buffer[1]).toBeInstanceOf(BlockCell);
    });

    it("parses parens", () => {
        const result = parse("(1 + 2)");
        const paren = result.buffer[0];
        expect(paren).toBeInstanceOf(ParenCell);
        expect(paren.buffer.length).toBe(3);
    });
    ...
    it("parses mixed content in blocks", () => {
        ...
        const block = result.buffer[0];
        expect(block.buffer.length).toBe(3);
        expect(block.buffer[0]).toBeInstanceOf(SetWordCell);
        expect(block.buffer[1]).toBeInstanceOf(NumberCell);
        expect(block.buffer[2]).toBeInstanceOf(StringCell);
    });
});
...
describe("Parser - Paths", () => {
    it("parses simple paths", () => {
        ...
        const path = result.buffer[0];
        expect(path).toBeInstanceOf(PathCell);
        expect(path.buffer.length).toBe(2);
        expect(path.buffer[0]).toBeInstanceOf(WordCell);
        expect(path.buffer[1]).toBeInstanceOf(WordCell);
    });
    ...
    it("parses paths with numbers", () => {
        ...
        expect(path.buffer.length).toBe(2);
        expect(path.buffer[0]).toBeInstanceOf(WordCell);
        expect(path.buffer[1]).toBeInstanceOf(NumberCell);
        expect(path.buffer[1].value).toBe(1);
    });

    it("parses multi-segment paths", () => {
        const result = parse("a/b/c/d");
        const path = result.buffer[0];
        expect(path.buffer.length).toBe(4);
    });
    ...
    it("parses paths with mixed segments", () => {
        ...
        const path = result.buffer[0];
        expect(path.buffer.length).toBe(4);
        expect(path.buffer[2]).toBeInstanceOf(NumberCell);
    });
});
...
describe("Parser - Whitespace and Comments", () => {
    ...
    it("handles comments", () => {
        ...
        expect(result.buffer[0].value).toBe(1);
...
describe("Parser - Multiple Values", () => {
    it("parses multiple values", () => {
        ...
        expect(result.buffer[0]).toBeInstanceOf(SetWordCell);
        expect(result.buffer[1]).toBeInstanceOf(NumberCell);
        expect(result.buffer[2]).toBeInstanceOf(SetWordCell);
        expect(result.buffer[3]).toBeInstanceOf(NumberCell);
    });
...
describe("Parser - Complex Examples", () => {
    it("parses function definition", () => {
        ...
        expect(result.buffer[0]).toBeInstanceOf(SetWordCell);
        // func
        expect(result.buffer[1]).toBeInstanceOf(WordCell);
        // [x]
        expect(result.buffer[2]).toBeInstanceOf(BlockCell);
        // [x + x]
        expect(result.buffer[3]).toBeInstanceOf(BlockCell);
    });
    ...
    it("parses object literal", () => {
        ...
        const block = result.buffer[0];
        expect(block.buffer.length).toBe(4);
    });
});
