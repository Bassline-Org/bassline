 ...
 * @param {object} spec.bindings - Kit bindings for each node
 * @param {object} spec.ports - External port mappings
...
export const circuit = (spec, resources) => {
  ...
  for (const [name, res] of Object.entries(resources)) {
    ...
    // Build kit from bindings: { '/in': 'input' } → routes({ in: resources.input })
    const kit =
      Object.keys(nodeBindings).length > 0
        ? routes(
            Object.fromEntries(
              Object.entries(nodeBindings).map(([path, target]) => [path.replace(/^\//, ''), resources[target]])
            )
          )
        : undefined
  ...
  // Build port router
  return routes(
    Object.fromEntries(Object.entries(ports).map(([port, node]) => [port.replace(/^\//, ''), wrapped[node]]))
  )
}
