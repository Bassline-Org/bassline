...
Specifically we, at Bassline, are most interested in how this
simplifies decentralized distributed programming. An area that outside
of blockchains or very specific protocols is relatively
starved.
...
This is enabled not by something we built, specifically it's what we omit.
...
We don't believe distributed programming is a boogeyman, it's just
misunderstood. More like a particularly moody cat or an alligator.
...
express the program we want.

We believe that the uncontrolled proliferation of this idea (we refer
to this as non-local ontology) is EXTREMELY harmful in distributed
settings. The root problem is that, in a distributed setting, there
are literally no "things". A distributed program is just a program
running on a single machine that talks with the outside world via
local affordances. There is no "message broker", there is locally
received messages that I locally assign to be coming from a message broker.

Bassline doesn't fight this, in fact we lean into it fully. We
acknowledge that outside of our local programs, we literally can't
know anything. There is an extreme asymetry of information between
"what I know currently" and "literally everything else".

So rather than build from "things", we build from communcation, with
the commitment being we do not provide tools that that aren't also
available in the most hardcore p2p setting you can image. The result
...
* The Kernel
The kernel starts with messages and local capabilities for
communication. Keep in mind the important thing is the ability to
exchange and observe information, not any particular interface shape.
...
Messages are the most important concept in Bassline. So let's take a
moment to define what a message really is.
...
They do not have a declared schema other than being "dictionaries" at
the top level.

Because they are data a message is intert and you are free to be
replicated and shared as you wish.

**** Messages are anonymous

When we say a message is anonymous, it simply means we cannot address
a message.

A message can be bound to a name locally, giving us a way to refer to
it. But that is a property of the binding, not of the message.

You also might have a way of identifying a message such as an id or a
hash. But this not fundamental to a message.
...
meaning other than what meaning you choose to assign to it.
...
**** Messages can have capabilities

When passing messages around individuals may choose to enrich a
message with capabilities. A capability, or "cap" for short, is a
uni-directional opaque function that can travel alongside the data of
a message in-process.

Conceptually, it gives the anonymous message an anonymous way to speak
to something. Kinda like being handed a phone that's already on a
call, but they are on mute.

You can speak into it, they don't know who you are, since they dialed
"not-you", nor do you know who they are. But you are connected through
some chain of events that caused you to be able to speak with them.

TODO: Update this prose to be better.
...
As we established in the abstract section, messages are the most
important concept in bassline.

It contains data as a dictionary with string keys, as well as a
distinct space for capabilities in process.

When serializing or transporting messages, only the data is kept, and
caps must be reified as data in order to be mobile.

This implementation isn't really special, and you could implement an
alternative kind of Msg, but this works well and is very ergonomic
...
Messages have their own lifecycle. This is managed by the
=createController= construct we define.

When a message is closed, it's caps are cleared.

With the exception of =signal=, the message has the same interface as
the return from createController.

**** Data Access

Most of the access methods on the class are polymorphic.

If passed a string, they will do a singular version of something.
Otherwise if given an array, they will do it for each element of the array.

When it makes sense, most operations try to return =this=, to allow for
method chaining.

We omit a =set= method on the message, so if you want to change the
data, use merge which passes in an object, which is merged into the
data.
...
=invoke= is a no-op when a message lacks a capability. This can be
tested for if needed.

The only "sugar" included in the message class is =.send=, which
internally just uses =this.invoke('send', arg)=.

The reason this gets a pass is because it's the most common operation
we perform.

**** Manipulation

The message implementation exposes a =copy= method for when we want to
copy a message. It performs a shallow copy of the data, and shares
the caps of the copied message.

The =copy= method also takes an optional argument of data to merge into
the copy. Because most of the time when we copy, we are adding new
data to the message.

*IMPORTANT!* The resulting message has it's own lifecycle, meaning
closing the source message, has no impact on the copy.

In addition to this, we offer 2 helpful methods for misc message manipulation.

1. =do(aFn)=

   Takes a function, and passes this directly as the sole argument,
   returning whatever the function returns.

2. =map(aFn)=

   Similar to do, but passes a =copy= into the function so changes don't
   mutate this message.
...
The communication constructs in Bassline are implemented as functions
that return a tuple: =[message, locals]=

The =message= returned from the tuple includes all caps for the
primitive. Whereas locals, depending on the construct, is things that
aren't capabilities, such as =recv= for ports, =to= for propagators, etc.

The lifecycle of the communication construct is commonly tied to the
message. So when =message= closes, the communication construct is shutdown.

The fact that our communication constructs have a message represention
with caps, it means they can be shared as messages for others to use,
but you cannot send the =locals= as a message directly!
...
2. =close=
   ...
   *This is distinct from the =message.close()=!*
   ...
   It's important to note that there is no builtin lifecycle
   propagation with ports. It's trivial to include, but is leaky to
   include. This is because knowing how to "send" to something,
   doesn't mean we know it's alive, or that they can communicate with
   us.

   Senders are not notified and their messages are dropped should they
   send to the closed port.  This ensures neither side requires
   knowledge of the other's state / status without the other side
   explicitly sharing this, maintaining the sovereignty of the caps.

   You should still communicate when you die though but we can't force you.
   "#messaging-in-my-grave"
...
The only other thing to note is the EOF symbol. This symbol is sent
through and used by local port consumers to indicate that the port
closed.  It is not universal and will serialize over the wire
meaningfully.  It only exists to make our lives simpler in js.  And it
is trivial to build your own interpretation of what should signal EOF
locally.

You cannot pass EOF directly through a port's =send=, nor should you ever have
...
this takes 2 arguments: =aMsg= and =propagate=. Propagate simply forwards
the value to the targets. This allows us to propagate multiple times
per invocation.

You can also use a propagator without forwarding as a reactive
computation by just not calling propagate in the body.

A propagator's message exposes the same caps as a port, making them
behaviorally equivalent as messages. However unlike =port=, a propagator
doesn't buffer. It eagerly does work.

Cycles are supported and encouraged, just make sure you correctly
setup your propagators to ignore irrelevant information.

*NOTE: We might want to make =to= instead take messages rather than
functions, it's probably cleaner since we can do lifecycle stuff.*
...
This is the most common way to use a port. It takes a =recv= function
...
Every transport produces the same thing as ports do. They produce a message with
=send= and =close= caps. And a local =recv= function.

The only difference is the medium underneath and whether that
