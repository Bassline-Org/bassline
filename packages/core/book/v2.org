...
* Introduction
This literate document contains the source code for the core of [[https://bassline.dev][Bassline]] v2.5.
...
Specifically, at Bassline, we are most interested in how this
simplifies decentralized distributed programming. An area that outside
of blockchains or very specific protocols is relatively starved.
...
This is possible by what we choose to omit from our "Bassline", not by
what we include.
...
* The Kernel
:PROPERTIES:
:header-args: :tangle ../src/bassline.js :comments yes :padline yes
:END:
...
Because they are data and you are free to be replicated and shared as
you wish.
...
data. You can also use =copy= to do this for a freshly copied message.
...
In order for something to be a capability, it must be an opaque way of
speaking. Because this is the most we can commit to in a distributed
setting without leaking commitments that don't hold. As such, =recv= is
not allowed to be a capability. I cannot give you a way of receiving a
message because receiving a message is always through your local
affordances. The only thing we can do in a distributed setting, is communicate
ways of communicating through an existing medium.
...
*** Framing
:PROPERTIES:
:header-args: :tangle ../src/frame/jsonl.js :comments yes :padline yes
:END:
...
*** Socket
:PROPERTIES:
:header-args: :tangle ../src/transports/socket.js :comments yes :padline yes
:END:
...
#+begin_src js
  ...
  export function fromSocket(socket, frame = defaultFrame) {
    ...
    socket.on('data', chunk =>
      reader.send(
        msg().merge({ scalar: chunk.toString() })
      )
    )
...
*** Serving (TCP)
:PROPERTIES:
:header-args: :tangle ../src/serve/tcp.js :comments yes :padline yes
:END:
...
*** Serving (WebSocket)
:PROPERTIES:
:header-args: :tangle ../src/serve/ws.js :comments yes :padline yes
:END:
...
#+begin_src js :tangle ../src/transports/websocket.js :comments yes :padline yes
...
#+begin_src js :tangle ../src/transports/worker.js :comments yes :padline yes
  import { msg, port } from '../bassline.js'
...
#+begin_src js :tangle ../src/transports/stdio.js :comments yes :padline yes
