...
export function assertCondition(response, expectedCondition, expectedMessage) {
  if (response.headers.condition !== expectedCondition) {
    throw new Error(
      `Expected condition '${expectedCondition}' but got '${response.headers.condition}'`
    )
  }
  if (expectedMessage && response.headers.message !== expectedMessage) {
    throw new Error(
      `Expected message '${expectedMessage}' but got '${response.headers.message}'`
    )
  }
}
...
export function assertSuccess(response) {
  if (response.headers.condition) {
    throw new Error(
      `Expected success but got condition '${response.headers.condition}': ${response.headers.message}`
    )
  }
}
