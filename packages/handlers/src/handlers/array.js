...
export function registerArray({ registerBuiltin }) {
  ...
  registerBuiltin(
    'sort',
    (ctx) => (arr) => [...(arr || [])].sort(ctx.descending ? (a, b) => b - a : (a, b) => a - b)
  )
  ...
  registerBuiltin(
    'arraySum',
    () => (arr) => (Array.isArray(arr) ? arr.reduce((a, b) => a + b, 0) : null)
  )

  registerBuiltin(
    'arrayProduct',
    () => (arr) => (Array.isArray(arr) ? arr.reduce((a, b) => a * b, 1) : null)
  )
  ...
  registerBuiltin(
    'arrayMin',
    () => (arr) => (Array.isArray(arr) && arr.length ? Math.min(...arr) : null)
  )

  registerBuiltin(
    'arrayMax',
    () => (arr) => (Array.isArray(arr) && arr.length ? Math.max(...arr) : null)
  )
}
