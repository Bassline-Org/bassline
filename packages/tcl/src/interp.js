/**
 * This is the interpreter for our TCL language
 * The reason I say only interpreter and not just parser, is because they are basically the same thing in TCL.
 *
 * TCL is described in 12 rules known as the Dodekalogue, and it lacks a formal grammar.
 * Go to: https://wiki.tcl-lang.org/page/Dodekalogue to read more
 *
 * ======
 * Rules:
 * ======
 ...
 * Semicolons and newlines are command separators unless quoted as described below.
 * Close brackets are command terminators during command substitution (see below) unless quoted.
 *
 * ================
 * [2] Evaluation
 * ================
 ...
 * First, the Tcl interpreter breaks the command into words and performs substitutions as described below.
 * These substitutions are performed in the same way for all commands.
 * The first word is used to locate a routine to carry out the command,
 * then all of the words of the command are passed to the routine.
 * The routine is free to interpret each of its words in any way it likes, such as an integer, variable name, list, or Tcl script.
 ...
 * If semicolons, close brackets, or white space characters (including newlines) appear between the quotes
 * then they are treated as ordinary characters and included in the word.
 * Command substitution, variable substitution, and backslash substitution are performed on the characters between the quotes as described below.
 ...
 * then the leading “{*}” is removed and the rest of the word is parsed and substituted as any other word.
 * After substitution, the word is parsed as a list (without command or variable substitutions;
 * backslash substitutions are performed as is normal for a list and individual internal words may be surrounded by either braces or double-quote characters),
 * and its words are added to the command being substituted.
 *
 * ================
 * [6] Braces
 * ================
 *
 * If the first character of a word is an open brace (“{”) and rule [5] does not apply,
 * then the word is terminated by the matching close brace (“}”).
 * Braces nest within the word: for each additional open brace there must be an additional close brace
 * (however, if an open brace or close brace within the word is quoted with a backslash then it is not counted in locating the matching close brace).
 * No substitutions are performed on the characters between the braces except for backslash-newline substitutions described below,
 * nor do semi-colons, newlines, close brackets, or white space receive any special interpretation.
 * The word will consist of exactly the characters between the outer braces, not including the braces themselves.
 *
 * ================
 ...
 * The result of the script (i.e. the result of its last command) is substituted into the word in place of the brackets and all of the characters between them.
 * There may be any number of command substitutions in a single word. Command substitution is not performed on words enclosed in braces.
 *
 * ================
 * [8] Variable substitution
 * ================
 *
 * If a word contains a dollar-sign (“$”) followed by one of the forms described below, then Tcl performs variable substitution:
 ...
 * Name must contain only letters, digits, underscores, and namespace separators, and may be an empty string.
 * Letters and digits are only the standard ASCII ones (0-9, A-Z and a-z).
 ...
 * It may contain any characters whatsoever except for close braces.
 * It indicates an array element if name is in the form “arrayName(index)” where arrayName does not contain any open parenthesis characters,
 * “(”, or close brace characters, “}”, and index can be any sequence of characters except for close brace characters. No further substitutions are performed during the parsing of name.
 ...
 *  This allows characters such as double quotes, close brackets, and dollar signs to be included in words without triggering special processing.
 * The following table lists the backslash sequences that are handled specially, along with the value that replaces each sequence.
 ...
 * This backslash sequence is unique in that it is replaced in a separate pre-pass before the command is actually parsed.
 * This means that it will be replaced even when it occurs between braces, and the resulting space will be treated as a word separator if it isn't in braces or quotes.
 *
 * Backslash (“\”).
 * \ooo
 ...
 * If command substitution occurs then the nested command is processed entirely by the recursive call to the Tcl interpreter;
 * no substitutions are performed before making the recursive call and no additional substitutions are performed on the result of the nested script.
 ...
 * ================
 *
 * Substitutions do not affect the word boundaries of a command, except for argument expansion as specified in rule [5].
...
const isComment = matches(/\#/)
const isExpansion = matches(/\{\*\}\S/)
const isVarDelimiter = matches(/[ \t;\n$]/)
const isTrimmableStart = any(isHorizontalWs, isCommandSep, isComment)

class MissingProc extends Error {
  constructor(name) {
    super(`Failed to find proc with name: ${name}`)
  }
}
class InvalidProc extends Error {
  constructor(name, proc) {
    super(`Invalid proc ${name}: Must be a fn, got: ${typeof proc}`)
  }
}

class TclParser extends CharStream {
  constructor(runtime, script) {
    super()
    this.rt = runtime
    if (script) this.write(script)
  }

  _trimStart() {
    while (isTrimmableStart(this.peek())) {
      if (this.peek() === '#') {
        this.skipUntil(matches(/\n/))
        continue
      }
      this.next()
    }
  ...
  _readExpansion() {
    throw new Error('todo: expansion')
  }
  ...
  parseCommand() {
    ...
    // Trim whitespace to seek to the start of a command
    this._trimStart()

    // SECTION: loop for command parsing
    while (!this.done() && !isCommandSep(this.peek())) {
      // SECTION: loop for word parsing (delimited by horizontal whitespace)
...
class TclRuntime {
   ...
   * @param {String} name
   * @param {TclCommand} fn
   * @throws {InvalidProc} If not passed a function
   * @returns {String}
   */
  setProc(name, fn) {
    if (typeof fn !== 'function') throw new InvalidProc(name, fn)
    this.procs[name] = fn
    return name
  }
  /**
   ...
   * @throws {MissingProc} If we couldn't resolve the procedure
   * @returns any
   */
  _runProc(name, args) {
  ...
  run(script) {
    const parser = new TclParser(this, script)
    let command = parser.parseCommand()
    let res
