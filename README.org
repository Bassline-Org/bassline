...
* What
This is a project to experiment with novel interfaces for propagation networks. The most recent work on this was done by Gerald Sussman and Alexey Radul, for Radul's 2009 PhD thesis, where he made the important contribution that cells refine partial information, rather than handle complete values.
...
This merge function forms a semi-lattice structure, meaning that there are states that cannot be merged or reconsiled, which we call a contradiction. When we have two data values being merged into a single cell that are mutually exclusive, we simply "chuckle", which is to express that we have a contradition.
When a cell merges in information, if it didn't learn anything new nothing happens. However when it merges in information that results in a "higher state" in the semi-lattice, it will notify it's connected propagators that it's contents have changed.

For example, if a cell holds the value 5, but it receives a interval (1 to: 10), it didn't gain any information, since the value 5 is more refined and non-contradictory for the cells incoming content.
...
Propagators can have something called an "activation policy", which is some internal logic that determines whether or not the propagator should fire. The simple propagator will just fire any time it's inputs change, but you can imagine smarter activation policies.
...
1. Contacts
   ...
   Connections between contacts are valid so long as it's:
...
2. Groups (Gadgets)
   ...
   Note: The "primitive" propagators in our system, such as basic arithmetic propagators are implemented as groups that cannot be opened. However all groups can be opened and modified internal, however doing so changes the identity of the group, and is therefore not the same as the other group.
...
A lot of these metaphors are inspired by the interactions that dreams (the media molecule game) offers, because I think it's a good basis for what it means to interact with lively diagram systems.

So our system allows building compound data using "splitter" and "joiner" gadgets, where we can take multiple wires, connect as inputs, and output a single value, which wraps all of these together as one value in a "fat wire". Then we can do the inverse to extract elements from a fat wire.
