...
/**
 * Duplicate Action (Meta-Action)
 *
 * Targets another pending action on the stack.
 * When resolved, clones the target action.
 */
export function duplicateAction(): MetaAction {
  ...
  return {
    id: 'duplicate-action',
    name: 'Duplicate',
    description: 'Duplicate a pending action',
    isMeta: true,
        ...
        <div class="duplicate-overlay">
          <div class="meta-prompt">
            <div class="prompt-header">
              ...
              <span class="prompt-title">Duplicate Action</span>
            </div>

            <Show
              when={targets.length > 0}
              fallback={<div class="no-targets">No pending actions to duplicate</div>}
            >
              <div class="prompt-instruction">Click an action in the stack to duplicate it</div>
    ...
    async execute() {
      ...
      const duplicate = stackStore.duplicateAction(id)
      if (duplicate) {
        ctx?.toast.info(`Duplicated action`)
      }
    },
  }
}
