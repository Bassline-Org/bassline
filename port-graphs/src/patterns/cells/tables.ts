...
import { ExtractSpec, GadgetSpec, TypedGadget } from '../../core/types';
...
export type TableHelpers<State, Input, Changes> = {
    ...
    handleAdd: (newState: State, changes: Changes) => void;
    handleRemove: (newState: State, changes: Changes) => void;
}

export type TableSpec<State, Input, Changes> = {
    state: State;
    input: Input;
    actions: {
...
export const defTable = <State, Input, Changes>(
    helpers: TableSpec<State, Input, Changes>['helpers'],
) => {
    return (initial: Input) => {
        const initialData = helpers.setup(initial);

        const gadget = defGadget<TableSpec<State, Input, Changes>>(
            (state, input) => {
            ...
            },
            {
                merge: (gadget, { added, removed }) => {
                    ...
                    helpers.handleAdd(newState, added);
                    helpers.handleRemove(newState, removed);
...
export const handleAdd = <K extends keyof any, V>(newState: Record<K, V>, changes: Record<K, V>) => {
    for (const key in changes) {
        newState[key] = changes[key];
    }
}

export const handleRemove = <K extends keyof any, V>(newState: Record<K, V>, changes: Record<K, V>) => {
    for (const key in changes) {
        delete newState[key];
    }
}

export const detectChanges = <K extends keyof any, V>(state: Record<K, V>, input: Record<K, V>) => {
...
export const deriveTable = <S, Derived>(source: S, transform: (key: keyof S, value: S[keyof S]) => [keyof S, Derived]) => {
    type Spec = S extends TappableGadget<infer SourceSpec>
        ? SourceSpec
        : never;

    type Key = keyof Spec['state'];
    type State = Record<Key, Derived>;
    type Input = Record<Key, S[Key]>;
    type Changes = Record<Key, Derived>;
    const transformFn = transform as (key: Key, value: S[Key]) => [Key, Derived];
    ...
    return defTable<State, Input, Changes>({
        setup: (initial: Input) => {
            ...
            for (const key in initial) {
                ...
                const [derivedKey, derivedValue] = transformFn(key, value);
                initialState[derivedKey] = derivedValue;
            }
            return initialState;
        },
        detectChanges: (state, input) => {
            return detectTableChanges(state, input as Record<Key, Derived | null>);
        },
        handleAdd,
        handleRemove,
    })
