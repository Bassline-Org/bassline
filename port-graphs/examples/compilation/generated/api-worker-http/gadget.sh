#!/bin/bash
# Wiring script for relationship: api -> worker
# Protocol: http
...
PIPE_NAME="/tmp/api_to_worker_$PROTOCOL"
if [ ! -p "$PIPE_NAME" ]; then
    mkfifo "$PIPE_NAME"
fi
...
case "$1" in
    "start")
        ...
        monitor_connection &
        echo $! > "/tmp/wire_api_worker.pid"
        ;;
    "stop")
        if [ -f "/tmp/wire_api_worker.pid" ]; then
            kill $(cat "/tmp/wire_api_worker.pid")
            rm "/tmp/wire_api_worker.pid"
        fi
        ;;
    *)
