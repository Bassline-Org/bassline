#!/bin/bash
# Wiring script for relationship: worker1 -> worker2
# Protocol: tcp
...
PIPE_NAME="/tmp/worker1_to_worker2_$PROTOCOL"
if [ ! -p "$PIPE_NAME" ]; then
    mkfifo "$PIPE_NAME"
fi
...
case "$1" in
    "start")
        ...
        monitor_connection &
        echo $! > "/tmp/wire_worker1_worker2.pid"
        ;;
    "stop")
        if [ -f "/tmp/wire_worker1_worker2.pid" ]; then
            kill $(cat "/tmp/wire_worker1_worker2.pid")
            rm "/tmp/wire_worker1_worker2.pid"
        fi
        ;;
    *)
