#!/bin/bash
# Wiring script for relationship: coordinator -> worker2
# Protocol: http
...
PIPE_NAME="/tmp/coordinator_to_worker2_$PROTOCOL"
if [ ! -p "$PIPE_NAME" ]; then
    mkfifo "$PIPE_NAME"
fi
...
case "$1" in
    "start")
        ...
        monitor_connection &
        echo $! > "/tmp/wire_coordinator_worker2.pid"
        ;;
    "stop")
        if [ -f "/tmp/wire_coordinator_worker2.pid" ]; then
            kill $(cat "/tmp/wire_coordinator_worker2.pid")
            rm "/tmp/wire_coordinator_worker2.pid"
        fi
        ;;
    *)
